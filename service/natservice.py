...
class NATService(object):
    """
    iptables commands
    reference: Reference: https://www.digitalocean.com/community/tutorials/how-to-forward-ports-through-a-linux-gateway-with-iptables
    """
    ...
    def __map(self, protocol, domain, public_ip, client_ip, ttl):
        ...
        for port in domain_ip_mappings[domain]:
            accept_tcp_cmd = [protocol, '-A', 'FORWARD', '-i', natconfig['PUBLIC_INTERFACE'], '-o',
                              ...
                              '--dport', str(port), '-m', 'conntrack', '--ctstate', 'NEW', '-j', 'ACCEPT']
    ...
    def __revoke_rule(self, protocol, port, r, public_ip, client_ip):
        ...
        accept_tcp_cmd = [protocol, '-D', 'FORWARD', '-i', natconfig['PUBLIC_INTERFACE'], '-o',
                          natconfig['PRIVATE_INTERFACE'], '-s', client_ip, '-d', public_ip, '-p', 'tcp', '--syn',
                          '--dport', str(port), '-m', 'conntrack', '--ctstate', 'NEW', '-j', 'ACCEPT']
        ...
        port_to_server_dst_cmd = [protocol, '-t', 'nat', '-D', 'PREROUTING', '-i', natconfig['PUBLIC_INTERFACE'],
                                  '-s', client_ip, '-d', public_ip,
                                  '-p', 'tcp', '--dport', '80', '-j', 'DNAT', '--to-destination', r[0]]
